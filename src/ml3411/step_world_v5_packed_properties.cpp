...
namespace hpce{
	namespace ml3411{
		...
		void StepWorldV5PackedProperties(world_t &world, float dt, unsigned n)
		{
			...
			queue.enqueueWriteBuffer(buffProperties, CL_TRUE, 0, cbBuffer, &packed[0]); //copies a specified area from host (CPU) memory to device (GPU) memory
...
int main(int argc, char *argv[])
{
	...
	try{
		...
		hpce::ml3411::StepWorldV5PackedProperties(world, dt, n);
